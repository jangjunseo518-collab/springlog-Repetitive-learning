...
public class ActivityByCategoryValidator implements
    ConstraintValidator<ValidActivityByCategory, CreateActivityRequest> {
  ...
  public boolean isValid(CreateActivityRequest value, ConstraintValidatorContext context) {
    ...
    return switch (value.category()) {
      case LECTURE -> value.instructorName() != null && !value.instructorName().isBlank();
      case PRACTICE -> value.completionRate() != null;
      case READING ->  value.bookTitle() != null  && !value.bookTitle().isBlank();
    };
  }
}
